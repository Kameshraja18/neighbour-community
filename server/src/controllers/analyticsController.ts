...
export const getSafetyScore = async (req: Request, res: Response) => {
    try {
        ...
        const score = await SafetyScore.findOne({
            location: {
                $near: {
    ...
    try {
        ...
        const incidents = await Incident.find({
            ...
            location: {
                $geoWithin: {
                    $box: [
                        [parseFloat(swLng as string), parseFloat(swLat as string)],
                        [parseFloat(neLng as string), parseFloat(neLat as string)],
                    ],
                },
            },
...
export const getSafetyTrends = async (req: Request, res: Response) => {
    try {
            ...
            Incident.aggregate([
                {
                    $match: {
                        ...
                        'location.coordinates': {
                            $geoWithin: {
                                $centerSphere: [coordinates, parseInt(radius as string) / 6378100],
                            },
                        },
                    },
                },
        ...
        let trend = 'stable';
        if (thisWeek > avgLastMonth * 1.2) trend = 'increasing';
        else if (thisWeek < avgLastMonth * 0.8) trend = 'decreasing';
        ...
        res.json({
            ...
            byHour: byHour.map((h: any) => ({ hour: h._id, count: h.count })),
        });
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
...
async function calculateSafetyScore(coordinates: number[]) {
    const now = new Date();
    const monthAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
